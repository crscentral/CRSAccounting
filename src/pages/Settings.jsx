...
export default function Settings() {
  ...
  // Load the current user's full name for use in invite emails
  useEffect(() => {
    if (user?.id) {
  ...
  async function inviteMember() {
    ...
    // Send the invite email to the invited person
    supabase.functions.invoke('send-team-invite', {
      body: {
        invitedEmail: inviteEmail.trim().toLowerCase(),
        inviterName: fullName || user?.email || 'Your team admin',
        companyName: activeCompany.name,
        role: inviteRole,
      },
    }).catch((err) => console.error('Failed sending invite email:', err))
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {cards.map(c => (
          <button key={c.key} onClick={() => setTab(c.key)}
            className={`text-left rounded-xl border p5 ${tab === c.key ? 'border-navy-400 bg-navy-50' : 'border-slate-200 bg-white'}`}>
          ...
          {can(['owner', 'admin']) && (
            <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-6">
              ...
              <div className="flex flex-col sm:flex-row gap-2 mb-5">
                ...
                <button onClick={inviteMember} disabled={inviting} className="flex items-center justify-center gap-1.5 bg-navy-600 text-white text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-60">
