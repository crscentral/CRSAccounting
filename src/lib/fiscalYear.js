...
/** Returns { from, to } ISO date strings for the current month-to-date. */
...
export function resolvePeriodRange(periodType, opts = {}) {
  const { fiscalYearStartMonth = 1, n = 1, customFrom, customTo, today = new Date() } = opts
  switch (periodType) {
    case 'MTD': return getMTDRange(today)
